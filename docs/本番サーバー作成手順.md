...
`<uid>` と `<gid>` は節 1.5 で確認した `uid` と `gid` を利用します。
...
# 証明書を One Blog 用の場所にコピー
cp /etc/letsencrypt/live/<サーバードメイン>/*.pem /home/github/One-Blog/containers/one-server/ssl-cert/
# 所有者を変更
chown <uid>:<gid> /home/github/One-Blog/containers/one-server/ssl-cert/*
exit
```

### 2.5. コンテナー作成

`管理者ユーザー` に切り替えて以下のコマンドを実行しデプロイを実行する。
...
```text
ユーザー名 (leave blank to use 'github'):
メールアドレス:
Password:
Password (again):
```
